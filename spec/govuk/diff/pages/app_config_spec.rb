describe Govuk::Diff::Pages::AppConfig do
  ...
  it 'raises an error for a non existent key' do
    expect {
      config.no_such_key
    }.to raise_error Govuk::Diff::Pages::AppConfig::MissingKeyError, "No such config key 'no_such_key'"
  end

  it 'raises an error for a non-existent nested key' do
    expect {
      config.nested.no_such_nested_key
    }.to raise_error Govuk::Diff::Pages::AppConfig::MissingKeyError, "No such config key 'no_such_nested_key'"
  end
